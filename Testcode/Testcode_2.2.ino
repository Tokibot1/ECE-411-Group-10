...
// Updated menu system with scrolling support
// Added settings menu and game settings
// Add a new game mode: Snake Game
// Snake came incomplete
// IMPROTANT : Although it writes 0x78 on the back, SCREEN ADDRESS is 0x3C
...
void startCountdown() {
  ...
  for (int i = 3; i > 0; i--) {
    ...
    display.print(i);
    display.display();
    tone(buzzerPin, 1000, 100); // Play tone
    delay(1000);
  }
  display.clearDisplay();
  display.display();
...
void reactionGame() {
  ...
  while (currentRound < reactionGameRounds) {
    ...
    while (digitalRead(buttonPins[randomLED]) == HIGH) {
      // Waiting for correct button press
    }
    ...
    currentRound++;
    delay(500); // Wait 0.5 seconds before next round
  }
...
void adjustSetting(int settingIndex) {
  ...
  while (adjusting) {
    ...
    if (digitalRead(buttonPins[1]) == LOW) { // Increase
      *settingValue += (settingIndex == 2) ? -50 : 1; // For snake speed, decreasing value increases speed
    ...
    if (digitalRead(buttonPins[2]) == LOW) { // Decrease
      *settingValue += (settingIndex == 2) ? 50 : -1;
      if (*settingValue > maxValue) *settingValue = maxValue;
      if (*settingValue < minValue) *settingValue = minValue;
      delay(200);
    }
