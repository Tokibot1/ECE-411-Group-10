// Game Collection 3.3
// - Modified Reaction Game to display "Wrong click!" and end the game if the wrong button is pressed
// - Screen address is 0x3C

// NEW bug: the reaction game is going more rounds if the wrong button
// is pressed. The game should only go to the next round if the correct
// DIno game could pause


// Reverted debounce method to previous version
// Added walls in the Snake Game
// Moved score display to the upper right corner in Snake Game
// Screen address is 0x3C
...
const int ledPins[4] = {D10, D9, D8, D7};
...
void showMenu() {
  while (inMenu) {
    ...
    if (digitalRead(buttonPins[0]) == LOW) { // Select current option
      inMenu = false;
      delay(200); // Debounce delay
    }
  }
  ...
  while (gameRunning) {
    ...
    if (digitalRead(jumpButtonPin) == LOW && !isJumping) {
     ...
     // tone(buzzerPin, 1000, 100); // Play jump sound
      //digitalWrite(ledPins[0], HIGH); // Turn on LED to indicate jump, commeted, has delay
      delay(200); // Debounce delay
    }
...
void snakeGame() {
  ...
  int foodX = random(1, (SCREEN_WIDTH - 4) / 4) * 4;
  int foodY = random(1, (SCREEN_HEIGHT - 4) / 4) * 4;
  ...
  while (gameRunning) {
    // Handle input without delays
    if (digitalRead(buttonPins[0]) == LOW && direction != 1) { // Up
      nextDirection = 3;
    }
    if (digitalRead(buttonPins[1]) == LOW && direction != 3) { // Down
      nextDirection = 1;
    }
    if (digitalRead(buttonPins[2]) == LOW && direction != 0) { // Left
      nextDirection = 2;
    }
    if (digitalRead(buttonPins[3]) == LOW && direction != 2) { // Right
      nextDirection = 0;
    }

    // Update game state based on time
    if (millis() - lastUpdateTime > snakeGameSpeed) {
      ...
      if (snakeX[0] < 0 || snakeX[0] >= SCREEN_WIDTH || snakeY[0] < 0 || snakeY[0] >= SCREEN_HEIGHT) {
        gameRunning = false;
        tone(buzzerPin, 500, 500); // Game over sound
        break;
      }
      ...
      if (snakeX[0] == foodX && snakeY[0] == foodY) {
        if (snakeLength < snakeMaxLength) {
          snakeLength++;
          tone(buzzerPin, 1500, 100); // Eat food sound
          digitalWrite(ledPins[2], HIGH);
          delay(100);
          digitalWrite(ledPins[2], LOW);
        }
        // Generate new food position
        foodX = random(1, (SCREEN_WIDTH - 4) / 4) * 4;
        foodY = random(1, (SCREEN_HEIGHT - 4) / 4) * 4;
        score += 10;
      }
...
void adjustSetting(int settingIndex) {
  ...
  while (adjusting) {
    ...
    // Handle button input
    if (digitalRead(buttonPins[1]) == LOW) { // Increase
      if (settingIndex == 2) {
    ...
    if (digitalRead(buttonPins[2]) == LOW) { // Decrease
      if (settingIndex == 2) {
        *settingValue += step;
      } else {
        *settingValue -= step;
    ...
    if (digitalRead(buttonPins[0]) == LOW) { // Confirm
      adjusting = false;
